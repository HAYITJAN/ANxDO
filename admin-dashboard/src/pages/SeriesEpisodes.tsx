...
export default function SeriesEpisodes() {
  ...
  async function remove(epId: string) {
    ...
    setBusy(true);
    const res = await apiFetch(`/episodes/${epId}`, { method: "DELETE" });
    if (res.ok) {
  ...
  if (!movie || !movieId) {
    return (
      <div className="text-zinc-400">
        <p>{err || "Topilmadi"}</p>
        <Link to="/movies" className="mt-4 inline-block text-violet-400 hover:underline">
          ← Kinolar
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Link to="/movies" className="mb-2 inline-block text-xs text-violet-400 hover:underline">
        ← Kinolar
      </Link>
      <h2 className="font-display mb-1 text-lg font-semibold text-white">Qismlar: {movie.title}</h2>
      <p className="mb-6 text-sm text-zinc-500">
        Tur: {movie.type}. Har til uchun: saytdagi treyler/video va/yoki to‘liq qism uchun tashqi sayt havolasi.
      </p>
      ...
      <form onSubmit={save} className="mb-10 space-y-4 rounded-2xl border border-white/[0.08] bg-[#0c0a12]/60 p-5">
        <p className="text-sm font-medium text-zinc-300">{editingId ? "Tahrirlash" : "Yangi qism"}</p>
        <div className="grid gap-4 sm:grid-cols-3">
          <label className="block">
            <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-zinc-500">Raqam *</span>
            <input
              type="number"
              min={1}
              required
          ...
          <label className="block sm:col-span-2">
            <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-zinc-500">Nom (ixtiyoriy)</span>
            <input
              value={form.title}
        ...
        <div className="space-y-2 rounded-xl border border-white/[0.06] bg-[#07060b]/50 p-3">
          ...
          <p className="text-xs text-zinc-600">Kamida bitta maydon (sayt yoki tashqi). Bir xil til ikki marta bo‘lsa, oxirgisi qabul qilinadi.</p>
      ...
      <h3 className="mb-3 text-sm font-semibold text-zinc-400">Ro‘yxat ({episodes.length})</h3>
        ...
        {episodes.map((ep) => (
          ...
          >
            <div className="min-w-0">
              ...
              <span className="ml-2 text-sm text-white">{ep.title?.trim() || `Qism ${ep.episodeNumber}`}</span>
